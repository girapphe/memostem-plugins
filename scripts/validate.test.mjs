...
test('public validation rejects private application paths and credential-shaped values', async (t) => {
  await t.test('private path', async (subtest) => {
    ...
    const privatePath = path.join(temporaryRoot, 'apps', 'web');
    await mkdir(privatePath, { recursive: true });
    await writeFile(path.join(privatePath, 'route.ts'), 'export const privateRoute = true;\n');
...
test('public validation rejects symlinks from the distributable tree', async (t) => {
  ...
  await symlink(
    ...
    path.join(temporaryRoot, 'plugins', 'memostem', 'README-link.md'),
  );
